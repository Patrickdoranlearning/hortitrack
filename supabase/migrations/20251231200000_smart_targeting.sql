-- ================================================
-- SMART SALES TARGETING MIGRATION
-- ================================================
-- This migration creates:
-- 1. targeting_config - Configurable algorithm weights
-- 2. eircode_zones - Irish routing key reference data
-- 3. customer_order_patterns - Materialized view of order patterns
-- 4. v_smart_sales_targets - Enhanced targeting view
-- ================================================

-- ================================================
-- 1. TARGETING CONFIGURATION TABLE
-- ================================================
-- Store configurable weights for the targeting algorithm
-- org_id NULL = system defaults, specific org_id = org override
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'targeting_config'
    AND policyname = 'Users can view targeting config'
  ) THEN
    CREATE POLICY "Users can view targeting config"
      ON public.targeting_config
      FOR SELECT USING (org_id IS NULL OR public.user_in_org(org_id));
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'targeting_config'
    AND policyname = 'Users can update their org targeting config'
  ) THEN
...
-- Insert default probability weights (org_id NULL = system defaults)
INSERT INTO public.targeting_config (org_id, config_key, config_value) VALUES
(NULL, 'probability_weights', '{
...
-- Insert default route fit weights
INSERT INTO public.targeting_config (org_id, config_key, config_value) VALUES
(NULL, 'route_fit_weights', '{
  "same_routing_key": 10,
...
COMMENT ON TABLE public.targeting_config IS 'Configurable weights for the smart targeting algorithm';

-- ================================================
-- 2. EIRCODE ZONES REFERENCE TABLE
-- ================================================
-- Irish Eircode routing keys with geographic info
...
CREATE TABLE IF NOT EXISTS public.eircode_zones (
  ...
  lat numeric(9,6),  -- Zone centroid for map
  lng numeric(9,6)
);
...
COMMENT ON TABLE public.eircode_zones IS 'Irish Eircode routing keys with adjacency and geographic data';

-- ================================================
-- 3. CUSTOMER ORDER PATTERNS MATERIALIZED VIEW
-- ================================================
-- Precompute customer ordering patterns for efficient targeting

CREATE MATERIALIZED VIEW IF NOT EXISTS public.customer_order_patterns AS
WITH order_data AS (
  SELECT
    o.customer_id,
    o.org_id,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_customer_order_patterns_pk
  ON public.customer_order_patterns(customer_id);

CREATE INDEX IF NOT EXISTS idx_customer_order_patterns_org
  ON public.customer_order_patterns(org_id);

COMMENT ON MATERIALIZED VIEW public.customer_order_patterns IS 'Precomputed customer ordering patterns for targeting algorithm';

-- ================================================
-- 4. REFRESH FUNCTION AND TRIGGERS
-- ================================================
...
RETURNS TRIGGER AS $$
BEGIN
  -- Only refresh on significant status changes
...
-- Trigger on order status change to confirmed/delivered
-- Note: This may cause performance issues with high order volume
-- Consider using pg_cron exclusively if needed
DROP TRIGGER IF EXISTS trg_refresh_order_patterns ON public.orders;
CREATE TRIGGER trg_refresh_order_patterns
AFTER UPDATE OF status ON public.orders
FOR EACH STATEMENT
EXECUTE FUNCTION public.refresh_customer_order_patterns();

-- ================================================
-- 5. ENHANCED TARGETING VIEW
-- ================================================

CREATE OR REPLACE VIEW public.v_smart_sales_targets AS
WITH
-- Get probability weights (use org-specific or fall back to defaults)
config AS (
  SELECT
    COALESCE(
...
-- Active delivery zones (orders scheduled for next 7 days)
...
customer_scores AS (
  SELECT
    ...
    -- Route match data (best matching delivery)
    ...
    CASE
      WHEN cop.total_orders IS NULL OR cop.total_orders = 0 THEN 30 -- New customer baseline
      ELSE LEAST(100, GREATEST(0,
        -- Frequency match: how close to their ordering cycle
        CASE
        ...
        END +
        -- Seasonality: did they order in this week last year?
        CASE
    ...
    -- Calculate route fit score (0-20)
    CASE
      -- Same routing key as scheduled delivery
      ...
      WHEN adz.routing_key IS NOT NULL AND
        ...
        LEAST((cfg.route_weights->>'density_bonus_max')::int,
              adz.order_count * (cfg.route_weights->>'density_bonus_per_order')::int)
      -- Same county (fallback when no eircode)
...
SELECT
  cs.*,
  -- Combined priority score (probability + route fit weighted)
...
-- Exclude customers who ordered recently (don't pester them)
WHERE NOT EXISTS (
...
AND (
  cs.route_fit_score >= 3  -- Route opportunity (same county or better)
  OR cs.probability_score >= 50  -- Likely to order
  OR cs.total_orders IS NULL  -- New customer
  OR cs.last_order_at < (NOW() - INTERVAL '6 weeks')  -- Churn risk
)
ORDER BY
  -- Prioritize route matches, then probability
...
-- ================================================
-- 6. DELIVERY ZONES SUMMARY VIEW
-- ================================================
-- Aggregated view of active delivery zones for the zone cards

CREATE OR REPLACE VIEW public.v_active_delivery_zones AS
SELECT
...
-- ================================================
-- 7. SCHEDULED DELIVERIES VIEW (for map markers)
-- ================================================

CREATE OR REPLACE VIEW public.v_scheduled_deliveries_map AS
SELECT
  o.id as order_id,
  o.org_id,
  o.order_number,
