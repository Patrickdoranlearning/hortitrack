-- Update perform_transplant to accept either containers OR units directly
-- This allows bulk transplant wizard to pass units directly without needing parent size info
...
CREATE OR REPLACE FUNCTION public.perform_transplant(
  ...
  p_units integer DEFAULT NULL  -- NEW: If provided, use this directly instead of calculating from containers
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_units IS NOT NULL AND p_units > 0 THEN
    -- Direct units provided - use as-is
    v_child_units := p_units;
  ELSE
    -- Calculate from containers using parent batch size
    DECLARE
