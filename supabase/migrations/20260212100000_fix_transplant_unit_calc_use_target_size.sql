-- Fix perform_transplant: use TARGET size (p_size_id) instead of parent size
-- when calculating units from containers.
--
-- Previously the function used the parent batch's size cell_multiple, which is
-- wrong when transplanting to a different size (e.g. plug trays -> 1L pots).
-- The UI calculates computed units using the target size, so the RPC must match.
...
BEGIN
  ...
  -- Fetch target size (needed for phase determination AND unit calculation)
  ...
  -- Determine child units: use p_units if provided, otherwise calculate from
  -- containers using the TARGET size (not parent size)
  IF p_units IS NOT NULL AND p_units > 0 THEN
    v_child_units := p_units;
  ELSE
    -- FIX: Use target size cell_multiple, not parent size
    v_child_units := p_containers * COALESCE(v_target_size.cell_multiple, 1);
  END IF;
...
COMMENT ON FUNCTION public.perform_transplant(uuid, uuid, uuid, uuid, integer, uuid, date, text, boolean, integer) IS
  'Atomically performs a batch transplant. Uses TARGET size cell_multiple (not parent) for unit calculation. Uses ISO week for batch numbering.';
