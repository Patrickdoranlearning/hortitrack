-- Fix v_smart_sales_targets subquery issue
...
customer_scores AS (
  SELECT
    ...
    -- Route fit score
    CASE
      WHEN adz.routing_key IS NOT NULL THEN
        10 + LEAST(5, COALESCE(adz.order_count, 0))
  ...
  LEFT JOIN active_delivery_zones adz ON
    UPPER(SUBSTRING(REPLACE(ca.eircode, ' ', ''), 1, 3)) = adz.routing_key
...
SELECT
  ...
  cs.avg_order_value,
  cs.last_order_at,
  cs.avg_order_interval,
  cs.preferred_dow,
  cs.value_quartile,
  cs.last_interaction_at,
  cs.last_interaction_outcome,
  cs.suggested_delivery_date,
  cs.van_current_load,
  cs.zone_order_count,
  cs.probability_score,
  cs.route_fit_score,
  ...
  CASE
    WHEN cs.route_fit_score >= 10 THEN
      'On ' || COALESCE(cs.county, 'Unknown') || ' route. Van ' || COALESCE(cs.van_current_load::text, '?') || '/10 trolleys'
    ...
    WHEN cs.probability_score >= 70 THEN
      'High likelihood to order based on patterns'
    ELSE
      'Regular customer - avg order €' || COALESCE(ROUND(cs.avg_order_value)::text, '?')
